...
## 0.7.9 — «Рой» (2026-09-04)

Тема выпуска: **видно состояние, а не органы управления**. Рой из полутора
десятков агентов Заря показывать умела — но так, что читать приходилось кнопки,
а не строки; а стоило прокрутить ленту вверх, и работа пропадала с глаз совсем.

### Изменено

- **Кнопки строк волны — по наведению.** Восемнадцать агентов давали
  восемнадцать одинаковых «ОСТАНОВИТЬ» и столько же «В ФОН»: столбик кнопок, в
  котором терялось то, ради чего смотрят, — чем каждый занят. Место за ними
  сохраняется, поэтому строка при наведении не дёргается; клавиатура находит их
  табом, а нажатая остаётся на виду — «останавливаю…» должно быть видно.

- **Одинаковый инструмент уехал в шапку.** Рой чаще всего делает одно и то же, и
  колонка из четырёх `Grep` занимала место, ничего не различая. Разошлись —
  инструмент возвращается в строки, где он снова становится различием.

### Добавлено

- **«РАБОТАЮТ: N» в нижней полосе — видно всегда.** Волна живёт в ленте своей
  панели: прокрутил вверх — её нет, при сетке 2×2 три панели из четырёх не видно
  вовсе, и вопрос «оно ещё работает?» решался глазами по столам. Строка отвечает
  на него, считая рой и фоновые задачи по всему окну, и ведёт к панели с
  работой — тем же кругом, каким «ждут решения» водит к ждущим. Только счёт:
  подробности живут в ленте, второе такое же место было бы шумом, а не
  видимостью.

- **«Остановить всё».** Кнопки не было вовсе: рой из восемнадцати задач
  прекращали, обрывая весь ход. Показывается по наведению на волну и только
  когда задач больше одной — работа обычно идёт нормально, и звать к её
  прекращению постоянной кнопкой незачем.

### Исправлено

- **Проверка разрешений перестала молчать о том, чего не проверяет.** Сценарий
  заявлен как round-trip карточки (появилась → одобрили → инструмент отработал),
  но движок давно выполняет `echo` сам, минуя нашу карточку. Сценарий печатал
  «approved: false» и завершался успехом: проверка самого чувствительного
  механизма продукта годилась только на вид. Теперь команда с побочным
  действием и явное падение, если карточка не поднялась.

### Числа

- **1285 юнит-проверок в 90 файлах** (было 1278). Полный офлайн-прогон: 79
  сценариев, 0 падений; прогон волны вырос с 35 проверок до 51 — вся новая
  работа закрыта им, включая остановку роя одной кнопкой.

## 0.7.8 — «Слово» (2026-08-26)

Тема выпуска: **Заря держит слово о себе**. Прошлый выпуск научил её не врать о
чужом — о движке, о цене, о соседней панели. Этот — о её собственных
обещаниях: что нажимается, то работает; что подписано, так и называется; что
обещано одному движку, верно для всех.

### Добавлено

- **К ждущему агенту наконец ведёт дорога.** Счётчик в нижней полосе был просто
  текстом, клик по уведомлению лишь поднимал окно, а действия «перейти к
  ждущему» не существовало вовсе: человек читал «ждут решения: 2» и шёл искать
  панель глазами по столам. Теперь счётчик — кнопка, уведомление ведёт в свою
  панель, а `Alt+↓` обходит ждущих по кругу.

- **Ответ агента можно унести с собой.** «Копировать» читало только выделение
  терминала, которого в блочном режиме нет: человек выделял ответ мышью и
  получал неактивный пункт меню. Теперь копируется и выделение ленты, и весь
  ответ целиком — кнопкой у самого ответа, ИСХОДНЫМ markdown, потому что
  заголовки и списки нужны в заметках.

- **Авария больше не исчезает бесследно.** Сорванная отрисовка снимала всё
  дерево: пустое окно и ни строки о том, что случилось. Теперь — честная
  остановка с текстом ошибки и тремя кнопками, плюс журнал сбоев на диске
  (`userData/logs/errors.log`, полмегабайта и одно запасное поколение).

- **Панель знает, где она, — в Командной строке и в WSL.** Каталог замирал на
  том, с которого её открыли, и не двигался ни на один `cd`. Это не подпись под
  вкладкой: каталог панели агент получает как рабочую папку и как границу
  доступа к файлам. Каждой оболочке нашёлся свой шов — `cmd` печатает по своей
  воле только приглашение, туда и вписан путь; в WSL переменные доезжают через
  `WSLENV`. Путь переводится в вид, понятный Windows. Побочно WSL получил и
  блоки команд.

- **Окно помнит, каким его оставили** — размер, положение, развёрнутость. С
  одной оговоркой на обе стороны: прямоугольник, до которого на сегодняшних
  экранах не дотянуться, не восстанавливается И не записывается. Монитор
  отключают, ноутбук уносят из дока — окно, открытое вслепую по вчерашним
  координатам, выглядит как «программа не запустилась».

### Изменено

- **Модель — у каждой панели своя.** В каждом ходе КАЖДОЙ панели уезжала одна
  глобальная настройка: смена модели в одной панели молча переводила на неё и
  соседнюю, а подпись под строкой ввода говорила HAIKU ровно в тот миг, когда
  ход уходил на OPUS. Теперь беседа берёт снимок модели при создании, а
  настройка отвечает за то, чем начинать новые.

- **У панели одно имя.** В разделе «Агенты» строка подписывалась первыми
  сорока символами первого запроса, а в списке панелей та же панель звалась
  своим именем — два списка одних и тех же агентов под несвязанными именами.
  Адрес панели — то, что написано на вкладке; первый запрос остался
  содержанием, в подсказке.

- **Числа собрались в нижней полосе.** Заполнение контекста стояло в ряду чипов
  над строкой ввода, среди органов управления, и при сетке 2×2 отнимало место у
  самой строки. Оно переехало к лимитам подписки. В панели остаётся
  предупреждение с 80% — там заполнение перестаёт быть показателем и становится
  поводом решить, что просить дальше, и прочитать об этом надо у ТОЙ панели,
  которой это грозит.

- **Автопилот больше не спрашивает ни о чём.** Под ним был «пол»: `rm -rf`,
  `git push --force`, `DROP TABLE` и помеченное сервером как разрушающее
  показывались карточкой даже при снятых гейтах. Чип обещал «выполняет все
  инструменты сам, без подтверждений», а Заря иногда всё-таки спрашивала. Пол
  ловил только дословную форму команды — то же удаление за переменной, за
  алиасом или внутри скрипта проходило мимо, — то есть защитой не был и без
  изоляции ОС быть ей не мог; а тумблер, который иногда спрашивает, хуже обоих
  честных состояний. Теперь автопилот значит автопилот. Исключение одно:
  `AskUserQuestion` доходит до человека всегда — это вопрос К нему, одобрять
  там нечего. Признак необратимого остался подписью на карточке для тех, кто
  спрашивать не отказывался, и правило «до конца сессии» ему по-прежнему не
  выдаётся.

- **Первый экран говорит правду.** Он обещал, что Заря «никуда ничего не
  отправляет», хотя на старте она проверяет обновления на GitHub. Теперь
  названо и единственное исключение, и то, что его можно выключить. А
  «нашлись и готовы к запуску» стало «нашлись на этой машине (вход в каждый — в
  нём самом)»: установленный, но не залогиненный CLI готов не был.

### Исправлено

- **Сообщение вместо ответа на карточку больше не обрезает беседу.** Карточка
  разрешения ждала решения, человек писал в строку вместо нажатия — и панель
  молча подменяла себя пустой беседой. Разговор с неотвеченной карточкой
  оставался цел на диске и в списке «АГЕНТЫ · ЖДЁТ», но уходил с экрана без
  единой строки объяснения: снаружи это неотличимо от потери работы. Сходились
  два места: строка судила о занятости наоборот (одобренный гейт считала
  занятостью, ждущий решения — нет), а «завести новую беседу» забирало всё, что
  не подошло под «продолжить». Теперь написанное встаёт в очередь, и его видно.

- **Enter, которым отправляли сообщение, больше не одобряет висящий гейт.**
  Нашлось живым прогоном сразу после первой правки. Одно нажатие обслуживают
  два обработчика подряд: строка отправляет текст и чистит поле, а диспетчер
  клавиш окна, дойдя до своей очереди, видит поле ПУСТЫМ и по этому признаку
  говорит «да» команде, которую человек не читал. Решение теперь принимается по
  состоянию строки до обработки.

- **Четыре мёртвых нажатия в конфигурации по умолчанию.** Строка «ждёт вас» вела
  в панель, которой без IDE не существует; `Ctrl+P` находил файл и не открывал
  его; ошибка про ключ посылала во вкладку, которую список прячет;
  переключатель «Плотность интерфейса» не имел во всём коде ни одного
  потребителя — убран вместе с настройкой, потому что тумблер, который ничего
  не делает, это ложь.

- **Авария, которую они прятали.** Сторож мёртвых нажатий сразу окупился:
  действие «Панель блоков» при пустом списке роняло всё окно (React #185).

- **Сообщение коммита перестало читаться как команда.** `git commit`, в тексте
  которого перечислялись `rm -rf`, `git push --force` и `DROP TABLE`, получал
  карточку «это не отменить»: правила разбирали строку целиком вместе с телом
  heredoc. Подпись утверждала про безобидный коммит, что после него нет пути
  назад, — а подпись, которая врёт, хуже отсутствующей. Теперь гасится тело
  heredoc и `-m`, и только у самого `git commit`: в кавычках бывает и настоящая
  команда, `psql -c "DROP TABLE users"` по-прежнему распознаётся.

- **`claude login` наконец упомянут.** Любая беда движка по умолчанию выходила
  общим «Claude Code не запустился», хотя для Codex, Kimi и ACP такие подсказки
  были. Теперь разобраны два случая, где человек может помочь себе сам, а
  остальное отдаётся дословно.

- **Подпись модели вернулась на экран.** Она не рисовалась НИГДЕ: блок стоял под
  условием, которое не выполняется никогда, — панель работала на своей модели, а
  посмотреть на какой было негде. Вместе с ним удалены 220 строк мёртвой
  разметки и семь лишних подписок на состояние.

### Числа

- **1278 юнит-проверок в 90 файлах** (было 1231 в 87 на 0.7.7). Файлов меньше на
  один: `codexFloor.test.ts` ушёл вместе с функцией, которую снял отказ от пола.
- **Полный офлайн-прогон: 78 сценариев, 0 падений.** Голос вынесен отдельным
  набором — ему нужен файл речи, и он синтезирует его сам (7 проверок, 0
  падений). Живой набор против настоящего Claude Code — тоже отдельно, он
  тратит токены подписки.
- Появился раннер `scripts/run-all.mjs`: набор берётся с диска, а исключённое
  печатается вслух — молча сузившийся прогон читается как «всё проверено».
