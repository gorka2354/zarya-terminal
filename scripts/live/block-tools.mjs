...
const { app, page, userData } = await launchZarya({
  work: stand,
  /*
   * Записки включаем ради ПОЛНОГО состава сервера; блоки едут по своему
   * согласию — подача консоли (`ai.contextBlocks`) и `blocks.enabled`, оба по
   * умолчанию включены. Раздельность согласий проверяет раздел [5] ниже.
   */
...
try {
  ...
  section('[4б] «last» — прочитать последнее без предварительного списка')
  /*
   * Самый частый случай: человек только что запустил команду и спрашивает, что
   * там. Без «last» это стоило вызова со списком, карточки на него и лишнего
   * хода. Карточку на само чтение это не отменяет — она называет команду,
   * которую прочтут (проверяет scripts/read-block-card-test.mjs).
   */
  await page.evaluate(
    ([id]) =>
      window.__zaryaSendTo?.(
        id,
        'Позови mcp__zarya__read_block с id "last" — без предварительного списка. Назови команду, вывод которой получил. Больше ничего не вызывай.'
      ),
    [a]
  )
  await waitIdle(page, a, 300_000)
  await page.waitForTimeout(2500)
  const t6 = await answer(page, a)
  note('ответ:', JSON.stringify(t6.slice(-320)))
  ok('прочитал последнюю команду без списка', /git log/i.test(t6), t6.slice(-320))
