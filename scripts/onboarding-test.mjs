...
try {
  ...
  /*
   * ЗДЕСЬ ПРОВЕРКА ДЕРЖАЛА НЕПРАВДУ. Она требовала дословного «никуда ничего не
   * отправляет» — а Заря на старте ходит на GitHub за обновлением (настройка
   * включена по умолчанию). Разведка это назвала; правило переписано так, чтобы
   * зелёным был честный текст, а не прежний.
   */
  ok(
    'сказано, что разговоры и настройки остаются на машине',
    /остаются на этой машине/i.test(local),
    local.slice(-140)
  )
  ok(
    'и названо единственное исключение — проверка обновлений',
    /обновлени/i.test(local) && /выключить/i.test(local),
    local.slice(-140)
  )
