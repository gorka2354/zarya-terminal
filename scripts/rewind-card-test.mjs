...
await withApp('ok', async (page) => {
  ...
  // Отметка остаётся в ленте — карточку закроют, а знать, что откат был, надо.
  const feed = await text(page, '.zy-mf-notice')
  ok('в ленте осталась отметка об откате', /Файлы откачены/.test(feed), feed)
  // И в ней НЕТ путей: лента уезжает на диск и в стенограмму, которую человек
  // отдаёт коллеге вместе с именами чужих проектов.
  ok('в отметке нет путей', !/[\/]/.test(feed.replace(/\s+/g, ' ')), feed)
