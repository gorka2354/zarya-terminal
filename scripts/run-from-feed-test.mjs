...
const run = (page, sid, code) =>
  page.evaluate(
    // Ровно как кнопка «запустить»: каждая строка получает свой Enter. Отправка
    // одним куском давала ОДИН блок со склеенными и переставленными командами.
    ([s, c]) => window.zarya.pty.write(s, c.trim().replace(/\r?\n/g, '\r') + '\r'),
    [sid, code]
  )
...
try {
  ...
  // ДВА блока, а не один склеенный: владелец увидел «cargo run cd C:\…» одной
  // строкой и прочитал это как поломку — лента показывала не то, что произошло.
  ok('блоков прибавилось на два', два.length === было + 2, { было, стало: два.length })
  const первый = два.findIndex((b) => /первая-строка/.test(b.cmd))
  const второй = два.findIndex((b) => /вторая-строка/.test(b.cmd))
  ok('первая строка — свой блок', первый >= 0, два.slice(-3))
  ok('вторая строка — свой блок', второй >= 0, два.slice(-3))
  // Что набрано первым, стоит выше: лента читается как время.
  ok('порядок не перепутан', первый >= 0 && второй > первый, { первый, второй })
  ok(
    'команды не склеены в одну строку',
    !два.some((b) => /первая-строка[\s\S]*вторая-строка/.test(b.cmd)),
    два.slice(-3)
  )
