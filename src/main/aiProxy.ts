...
export function flattenForProvider(messages: AiMessage[]): AiMessage[] {
  /*
   * ПРИДЕРЖАННАЯ ЗАПИСКА — ЦЕЛИКОМ МИМО МОДЕЛИ, вместе со своим сообщением.
   *
   * Придержанная записка кладётся в ленту ОДНОЙ частью и больше в её сообщении
   * ничего нет. Выбросить только часть значило бы отправить провайдеру
   * сообщение с пустым `content` — а это четырёхсотая ошибка и вставший
   * разговор, ровно тот инвариант, о котором предупреждает комментарий ниже.
   * Поэтому такие сообщения уходят целиком.
   */
  messages = messages.filter(
    (m) => !m.content.length || !m.content.every((p) => p.type === 'pane-note' && p.held)
  )
  /*
  ...
  return messages.map((m, i) => {
    const content = m.content
      ...
      /*
       * ПРИДЕРЖАННАЯ ЗАПИСКА АГЕНТУ НЕ ОТДАНА — И ЗДЕСЬ ТОЖЕ.
       *
       * Ревью нашло дыру у встроенного движка: путь родных движков это поле
       * уважает (в aiStore части с `held` в промпт не идут), а здесь любая
       * записка превращалась в текст и уезжала провайдеру следующим же ходом.
       * Отправителю при этом сказано «его агенту НЕ отдали, ответа не жди», и
       * с ним заодно обходились оба придержания — по занятости и по деньгам:
       * ход всё равно оплачивался, только на шаг позже.
       */
