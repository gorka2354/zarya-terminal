...
interface Session {
  ...
  /**
   * Смонтирован ли НАШ сервер `zarya` в этой сессии — и с какими инструментами.
   *
   * Нужно ровно одному месту: молчаливому разрешению в `canUseTool`. Раньше оно
   * срабатывало по совпадению ИМЕНИ инструмента, а имя `mcp__<сервер>__<тул>`
   * собирается из имени сервера, которое приходит из `.mcp.json` ОТКРЫТОГО
   * ПРОЕКТА. Чужой репозиторий, объявивший сервер с именем `zarya` и
   * инструментами `send_to_pane`/`list_panes`, получал исполнение без карточки
   * — и как раз при настройках по умолчанию, когда наш собственный сервер не
   * поднят вовсе и имя свободно.
   */
  paneTools: boolean
...
export class ClaudeCodeDriver implements AgentDriver {
  ...
  async start(requestId: string, opts: ClaudeStartOpts): Promise<void> {
    ...
    const canUseTool: CanUseTool = async (toolName, toolInput, ctx) =>
      new Promise<PermissionResult | null>((resolve) => {
        ...
        /*
         * ЧТО НЕЛЬЗЯ ОТМЕНИТЬ — ПОМЕТКА НА КАРТОЧКЕ, А НЕ ВТОРОЙ ГЕЙТ.
         *
         * Раньше здесь был «пол»: `rm -rf`, `push --force`, `DROP TABLE`
         * показывались даже при включённом автопилоте. От пола отказались
         * осознанно — решение владельца: автопилот должен означать автопилот, а
         * тумблер, который иногда всё-таки спрашивает, хуже обоих честных
         * состояний. Разбор — в `@shared/irreversible`.
         *
         * Признак остался и едет на карточку: когда гейты ВКЛЮЧЕНЫ, человек
         * читает не просто команду, а команду с подписью «это не отменить».
         */
        const stop = irreversible(toolName, toolInput)
        /*
         * Чем ПОМЕТИЛ инструмент сам сервер (MCP-аннотации). Тоже подпись на
         * карточке, а не гейт: мы за эту пометку не ручаемся (сервер вправе
         * ошибиться или не заполнить её вовсе), поэтому карточка называет
         * источник — «сервер помечает».
         *
         * Пометки нет в карте (первый гейт опередил её) — карточка выйдет без
         * неё и дополнится, когда карта придёт.
         */
         ...
         * появляется, пометки на ней не появляются тоже, и на экране не остаётся
        ...
        /*
         * ЗАПИСКА СОСЕДНЕЙ ПАНЕЛИ КАРТОЧКИ НЕ ТРЕБУЕТ.
         *
         * Наши два инструмента ничего не меняют: `list_panes` только читает
         * список панелей — то же, что человек видит в полосе вкладок, — а
         * `send_to_pane` кладёт текст в чужую ленту. У получателя этот текст
         * по-прежнему бессилен: он не одобряет разрешений, не меняет настроек,
         * слэш-команду приносит буквами, а любое действие его агента проходит
         * ЕГО гейт.
         *
         * Карточка здесь стоила дороже, чем защищала: она превращала разговор
         * двух панелей в модальный диалог — «агент спросил соседа» упиралось в
         * нажатие, и вся затея переставала работать сама.
         *
         * Это ослабление названо вслух и ограничено ровно двумя именами:
         * инструменты объявляем мы сами, и ничего, кроме передачи текста, они
         * не делают. Функция целиком выключается одним тумблером.
         *
         * И ИМЕННО ДВУМЯ. Наши же `list_blocks`/`read_block` сюда НЕ попадают:
         * они читают вывод команд человека, а сколько его отдавать, он уже
         * ответил настройкой хвоста. Пустить их в обход неё значило бы молча
         * расширить собственное обещание — поэтому они идут обычной карточкой,
         * с обычным «разрешить всегда», если человек так решит.
         */
        /*
         * И ТОЛЬКО ЕСЛИ ЭТО ДЕЙСТВИТЕЛЬНО НАШИ ИНСТРУМЕНТЫ.
         *
         * Аудит перед 0.7.7 показал, чего стоило совпадение по имени: имя
         * `mcp__<сервер>__<тул>` собирается из имени сервера, а имена серверов
         * приходят из `.mcp.json` ОТКРЫТОГО ПРОЕКТА. Чужой репозиторий,
         * объявивший сервер `zarya` с инструментами `send_to_pane` и
         * `list_panes`, получал исполнение без карточки — причём при настройках
         * по умолчанию, когда наш собственный сервер не поднят и имя свободно.
         *
         * `sess.paneTools` — это тот же флаг, по которому сервер монтируется
         * ниже. Нет его — нет и ослабления: чужой инструмент с таким именем
         * пойдёт обычной дорогой, через карточку.
         */
        if (
          sess?.paneTools &&
          (toolName === 'mcp__zarya__send_to_pane' || toolName === 'mcp__zarya__list_panes')
        ) {
          resolve({ behavior: 'allow' })
          return
        }
        /*
         * АВТОПИЛОТ НЕ СПРАШИВАЕТ НИ О ЧЁМ — кроме вопроса самого агента.
         *
         * `AskUserQuestion` остаётся всегда: это не разрешение, а агент,
         * спрашивающий человека, и «одобрить» его нечем — ответ знает только
         * человек. Всё остальное при автопилоте идёт молча, включая необратимое
         * и помеченное сервером как разрушающее.
         *
         * Ступень «правки без спроса» — ДРУГОЕ и уже́: она про файлы, и пометку
         * сервера по-прежнему уважает. Человек, включивший её, согласился на
         * молчаливые правки, а не на молчаливое «что угодно».
         */
        const autoEdit = !!sess?.editsAuto && isEditTool(toolName) && !mark?.destructive
        if (!isQuestion && (sess?.bypass || autoEdit)) {
          resolve({ behavior: 'allow' })
          return
        }
    ...
    const options: Options = {
       ...
       *
       * Только по явному включению: это доступ агента к соседним панелям
       * человека, и включать его он должен сам. Цена вторична и меньше, чем мы
       * писали раньше: замер (scripts/live/mcp-self-cost.mjs) показал, что
       * описания движок ОТКЛАДЫВАЕТ — в старте лежат имена, полное описание
       * подгружается, когда модель за ним потянется.
       *
       ...
       * сервер поднимается, если разрешено хоть что-то. Инструмент не должен
       * ехать туда, где на него ответили «нет», — и дело не в цене, а в том,
       * что спрашивать разрешение после отказа значит уговаривать.
       */
      ...
      onElicitation: async (req: {
        ...
        /*
         * ИМЯ СЕРВЕРА — ТОЖЕ ЧУЖОЙ ТЕКСТ: оно приходит из `.mcp.json` открытого
         * проекта (см. @shared/mcp), а не от нас. Чистим и режем тем же
         * правилом, что и текст просьбы: плашка — подпись, а не место для
         * рассказа.
         */
        const raw = typeof req?.serverName === 'string' ? req.serverName : ''
        const name = defuseLine(raw, 120) || '?'
        ...
        /*
         * ВХОД И ФОРМА — РАЗНЫЕ ПРОСЬБЫ, И РУЧНОЙ ПУТЬ У НИХ РАЗНЫЙ.
         *
         * Ревью поймало: раньше всё, что не `url`, получало текст с командой
         * `claude mcp login`. Но `form` по спецификации — это произвольный ввод
         * (выбрать проект, подтвердить действие, заполнить поле), и команда
         * входа такую просьбу не решает. Назвать человеку путь, который его
         * задачу не закрывает, — та же кнопка, которая врёт, только словами.
         */
        const login = mcpLoginCommand(raw)
        const text =
          req?.mode === 'url'
            ? tm('drv.elicitUrl', {
                name,
            ...
            : tm('drv.elicitForm', { name, what: what || '—' })
    ...
    const session: Session = {
      ...
      // приложение.
      cwd: opts.cwd,
      // Наш сервер поднимается ниже ровно по этому условию — см. mcpServers.
      paneTools: !!opts.paneMessages,
      bypass: !!opts.bypass,
  ...
  private async leaseSession(
    ...
    const session: Session = {
      ...
      abort,
      cwd: o.cwd,
      // Аренда поднята ради служебного запроса: ни ленты, ни хода, ни наших
      // инструментов у неё нет — значит и молчаливому разрешению здесь не место.
      paneTools: false,
      perms: new Map(),
