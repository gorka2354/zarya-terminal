...
export class FakeAgentDriver implements AgentDriver {
  ...
  async start(requestId: string, opts: AgentStartOpts): Promise<void> {
    ...
    if (mute) {
      ...
      // и подпись на карточке, и тишина под автопилотом проверяются только
      // настоящим необратимым вызовом.
      ...
      // разрушающий. Пометка едет на карточку подписью; отдельным гейтом она,
      // как и необратимое, больше не работает.
      const viaMcp = /mcp/i.test(opts.prompt)
      // Автопилот здесь ведёт себя как у настоящих движков: молча проходит ВСЁ,
      // включая необратимое. Прогонам нужен именно этот путь — иначе фейк
      // спрашивал бы там, где живые движки уже не спрашивают.
