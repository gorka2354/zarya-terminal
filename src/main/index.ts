  ...
  agentRegistry.set(
    ...
    new FakeAgentDriver(
      'codex',
      {
        ...
        directories: true,
        // Откат ФАЙЛОВ — только у этого фейка. У соседнего (gemini) кнопки не
        // должно быть вовсе: кнопка отката там, где откатывать нечем, — самая
        // дорогая из возможных, потому что человек на неё рассчитывает.
        rewindFiles: true,
