...
export const AgentBar = memo(function AgentBar({
   ...
   *
   * Сперва я поставил здесь запасной вариант: нет своего числа — берём общее по
   * окну. Разобрал по шагам и выбросил. Общее значение перезаписывает любая
        ...
        */}
        ...
        {!isShell && (
          ...
          >
            ...
            >
              {/*
                ТРИ СТУПЕНИ ДОПУСКА — ТРИ РАЗНЫХ ГЛИФА.
                Прежде их было два: молния на автопилоте и ОДИН И ТОТ ЖЕ замок
                на «спрашивать всё» и «правки без спроса». Различал их только
                цвет чипа, и человек, который цвета не различает, читал два
                разных обещания как одно. А обещание тут самое дорогое из всех:
                спросят ли меня перед тем, как тронуть файлы.
                Карандаш выбран не для красоты: «правки без спроса» — это ровно
                про правки, и глиф называет то, что ослаблено.
              */}
              {gateOff ? (
