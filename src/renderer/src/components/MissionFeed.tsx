...
const ToolCard = memo(function ToolCard({
  ...
  if (result) {
    ...
    // выдаётся вообще (такое решают по одному разу), и агентам, у которых своё
    // «разрешить всегда», второе такое же не нужно.
    const rule = always || stop ? null : ruleFor(pending.name, pending.input)
    body = (
      <div className="zy-mf-tool-actions">
        {stop && (
          /* Цена решения, названная до нажатия: после этой команды возврата
             нет. Не объяснение вопроса — сам вопрос человек уже выбрал, оставив
             гейты включёнными. */
