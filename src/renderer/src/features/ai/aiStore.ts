...
function answerBlocksQuery(q: Record<string, unknown>): void {
   ...
   *
   * Блоки собирает интеграция оболочки, и в SSH-панели или в Командной строке
   * её нет намеренно (cmd.exe сообщает КАТАЛОГ — через приглашение, — но о
   * командах не рассказывает: рассказывать ей нечем). Пустой список агент прочитает единственным доступным ему
