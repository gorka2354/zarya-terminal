...
/**
 * Занята ли беседа настолько, что новое сообщение человека не может начать ход.
 *
 * Одобренный гейт (`settled`) отсюда НЕ выпадает: решение принято, но результат
 * ещё не пришёл, и сообщение легло бы между `tool_use` и его `tool_result` —
 * такую ленту провайдер отвергает, а беседа портится необратимо.
 */
...
/**
 * Куда девать то, что человек написал в строку панели.
 *
 * ПОВОД. Строка знала два исхода — «продолжить свою беседу» и «завести новую» —
 * и второй забирал ВСЁ, что не подошло под первый: чужой движок, идущий ход,
 * висящий гейт. Написать что-нибудь вместо ответа на карточку разрешения
 * означало завести вторую беседу: панель молча переключалась на пустую, а
 * разговор с неотвеченной карточкой уходил с экрана целиком. Человек видел
 * обрезанную ленту, «готов · введите запрос» под ней и ни строчки объяснения —
 * при том что беседа была цела и ждала одного нажатия.
 *
 * Занятость и чужой движок — разные вещи, и здесь у них разные исходы:
 * `queue` — беседа та же, но ход идёт или гейт ждёт: приписка отправится, когда
 *           беседа освободится (её видно, её можно забрать назад стрелкой вверх);
 * `new`   — беседы нет вовсе или она чужого движка: новая законна;
 * `continue` — беседа своя и свободна.
 */
export type BarTarget = 'continue' | 'queue' | 'new'

export function barTarget(
  conv: Conversation | undefined,
  engine: 'builtin' | AgentEngine
): BarTarget {
  if (!conv || conv.engine !== engine) return 'new'
  return isConversationBusy(conv) ? 'queue' : 'continue'
}

/**
 * SECURITY: одобряет ли ГОЛЫЙ Enter висящий гейт.
 *
 * Одно нажатие обслуживают двое, и в этом порядке: сперва обработчик строки
 * ввода, потом диспетчер клавиш окна. Строка отправляет написанное и чистит
 * поле — и диспетчер, дойдя до своей очереди, видит поле уже ПУСТЫМ. По этому
 * признаку он одобрял гейт: человек написал сообщение, нажал Enter и тем же
 * нажатием молча сказал «да» команде, которую не читал. Ровно то слепое
 * согласие, ради которого гейты и существуют.
 *
 * Поэтому решает состояние строки ДО обработки (`hadText`), а не после.
 */
export function enterApprovesGate(o: {
  /** Есть гейт, ждущий решения (`nextGate`). */
  hasGate: boolean
  /** Курсор в чужом поле — Enter там означает «закончить здесь», а не «запускай». */
  inOtherField: boolean
  /** В строке что-то было в момент нажатия — значит это была отправка, а не «да». */
  hadText: boolean
}): boolean {
  return o.hasGate && !o.inOtherField && !o.hadText
}
