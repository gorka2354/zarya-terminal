...
async function spawnSession(
  ...
  // Новая панель ЗАПОМИНАЕТ режим, с которым родилась, вместо того чтобы вечно
  // следовать за общим умолчанием.
  //
  // Разница видна, когда панелей несколько. Пока у панели не было своей записи,
  // она читала `barMode` — а его меняет переключение в ЛЮБОЙ другой панели. То
  // есть человек переводил одну панель на Codex, и все, которых он не трогал,
  // молча переезжали следом. Панель — это отдельный CLI: соседняя не должна
  // менять то, что в ней запущено, а «как последняя» остаётся умолчанием ровно
  // на момент рождения.
  //
  // Только НОВОЙ: у восстановленной панели режим уже свой, из снапшота, и
  // подставить сюда общее умолчание значило бы стереть вчерашний выбор — ровно
  // то, ради устранения чего режим и начали хранить.
  if (!useUiStore.getState().barModeBySession[session.id]) {
    setPaneBarMode(session.id, useUiStore.getState().barMode)
  }
