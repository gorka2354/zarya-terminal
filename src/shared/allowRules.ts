...
export function ruleFor(toolName: string, input: unknown): AllowRule | null {
  /*
   * Необратимое не разрешается «до конца сессии» ВООБЩЕ — и это сознательно
   * пережило отказ от пола под автопилотом.
   *
   * Автопилот — режим, который человек включает целиком и видит на чипе: он
   * сказал «делай всё сам». А это правило действует, когда гейты ВКЛЮЧЕНЫ, то
   * есть человек как раз хочет вопросы; одно нажатие на карточке `rm -rf`
   * молча выключало бы их до конца сессии для всех таких команд. Разные вещи,
   * и решение здесь другое.
   */
...
export function matchesRule(
  ...
  // Симметрично `ruleFor`: правило на необратимое не создаётся, но проверяем и
  // здесь — правило могло приехать из сохранённого состояния прошлой версии.
