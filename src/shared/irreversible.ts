...
/**
 * Затереть пробелами, сохранив длину и переводы строк.
 *
 * Длина важна: индекс совпадения потом берётся из ИСХОДНОЙ строки — `hit`
 * показывается человеку и обязан быть настоящим куском его команды.
 */
...
/**
 * СООБЩЕНИЕ КОММИТА — НЕ КОМАНДА.
 *
 * Поймано на живом случае: `git commit`, в сообщении которого перечислялись
 * `rm -rf`, `git push --force` и `DROP TABLE`, получил карточку «это не
 * отменить». Правила читали строку целиком вместе с телом heredoc — и подпись
 * утверждала про безобидный коммит, что после него нет пути назад. Подпись,
 * которая врёт, хуже отсутствующей: следующую такую же человек уже не читает.
 *
 * ГАСИМ УЗКО, И ЭТО ВАЖНЕЕ УДОБСТВА. Первая версия убирала кавычки везде — и
 * тут же похоронила `psql -c "DROP TABLE users"`, где в кавычках как раз
 * команда. Разницу между «данные» и «код» в кавычках снаружи не видно, поэтому
 * гасится только то, про что мы знаем наверняка: тело heredoc и `-m`/`--message`
 * У САМОГО `git commit`. Всё остальное разбирается как прежде.
 */
function withoutText(cmd: string): string {
  if (!/\bgit\b[^|;&]*\bcommit\b/i.test(cmd)) return cmd
  return cmd
    .replace(/<<-?\s*(['"]?)([A-Za-z_][A-Za-z0-9_]*)\1[\s\S]*?^\2\b/gm, blank)
    .replace(/(?:-m|--message)\s*(['"])[\s\S]*?\1/gi, blank)
}

/**
 * Попадает ли вызов инструмента под признак необратимого.
 *
