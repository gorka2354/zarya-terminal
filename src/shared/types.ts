...
/**
 * Блок в том виде, в каком его видит АГЕНТ (`list_blocks` / `read_block`).
 *
 * Живёт здесь, а не рядом с мостом, потому что пересекает IPC: собирает его
 * окно, читает главный процесс, и две копии одной формы разошлись бы молча.
 *
 * Вывода тут нет намеренно — кроме короткого отрывка при поиске: список команд
 * должен стоить дёшево, а за выводом есть отдельный вызов с отдельной
 * карточкой разрешения.
 */
export interface BlockBrief {
  ...
  endedAt?: number
  /**
   * Сколько строк совпало с искомым. Только при поиске.
   *
   * Число нужно рядом с отрывком: одна совпавшая строка из сорока и одна из
   * одной — разный ответ на вопрос «стоит ли читать блок целиком».
   */
  matches?: number
  /**
   * Первая совпавшая строка, обрезанная. Только при поиске.
   *
   * ЧУЖОЙ ТЕКСТ, как и весь вывод: чистится на границе с моделью, там же, где
   * вывод `read_block`, — а не здесь.
   */
  snippet?: string
}
...
export interface AiSettings {
  ...
  /**
   * Разрешить агенту видеть соседние панели и писать им записки.
   *
   * ВЫКЛЮЧЕНО ПО УМОЛЧАНИЮ, и это не осторожность ради осторожности: агент
   * получает доступ к соседним панелям человека, а такое включают осознанно.
   *
   * Цена тут не главный довод, хотя раньше был написан именно он. Замер
   * (scripts/live/mcp-self-cost.mjs) показал, что описания инструментов движок
   * откладывает: в контексте лежат имена, полное описание подгружается по
   * надобности.
   */
...
export type AiContentPart =
  ...
  /**
   * Записка от СОСЕДНЕЙ ПАНЕЛИ — отдельный вид, и это главное решение inc-41.
   *
   * Нарисовать её репликой человека значило бы заставить его поверить, что он
   * сам это написал, — худшее, что можно сделать в проекте, чей принцип
   * «интерфейс не врёт». Поэтому у неё своё имя отправителя, свой вид и своя
   * пометка, когда её придержали.
   */
  | {
      type: 'pane-note'
      /** Имя панели-отправителя, как его видит человек. */
      from: string
      text: string
      /**
       * Придержана: агенту НЕ отдана. Причина названа словом, потому что она
       * меняет смысл для человека:
       *
       * - `autopilot` — у получателя выключены вопросы, и чужой текст стал бы
       *   действиями без единого спроса. Это решение о безопасности.
       * - `busy` — панель в середине хода: вклиниться между вызовом инструмента
       *   и его результатом нельзя, разговор бы испортился. Это про момент.
       * - `budget` — переписка этой пары панелей за час наработала больше, чем
       *   Заря готова начать без человека. Записка ПОКАЗАНА, но платного хода
       *   по ней не будет: продолжить может человек.
       */
      held?: 'autopilot' | 'busy' | 'budget'
    }
  /**
   * ХВОСТ КОНСОЛИ: последние команды человека, уехавшие агенту вместе с ходом.
   *
   * Своим видом — по той же причине, что и записка соседа. Текстом с пометкой
   * `[Контекст: …]` его делать нельзя: в этом репо такой префикс формально
   * означает «спрятать» (`SERVICE_PREFIXES` в @shared/turnId, фильтр в ленте),
   * и видимый след, ради которого всё затевалось, исчез бы молча.
   *
   * `text` собран НА МОМЕНТ ОТПРАВКИ и хранится готовым: пересобирать его при
   * перерисовке значило бы показывать сегодняшнюю консоль под вчерашним ходом.
   */
  | {
      type: 'shell-tail'
      /** Готовый текст для модели — с обёрткой недоверенного вывода. */
      text: string
      /** Команды, вошедшие в него, — для плашки в ленте. */
      used: Array<{ command: string; exitCode?: number }>
      /** Сколько блоков не влезло в потолок. */
      dropped?: number
    }
  /**
   * БОКОВОЙ ВОПРОС: спросили и не потащили дальше.
   *
   * Метка стоит на паре «вопрос — ответ», которая останется в ленте, но не
   * поедет в контекст следующего хода: он пойдёт веткой от точки ДО вопроса.
   *
   * Своим видом, а не текстом, по той же причине, что записка соседа: человек
   * должен видеть ГРАНИЦУ. Обещание «это не потащит контекст» без видимой
   * границы проверить нечем, а необеспеченное обещание — то же враньё.
   *
   * `kept` говорит правду о том, СБЫЛОСЬ ли обещание: движок мог не дать точку
   * (первый ход в сессии, движок без форка). Тогда пара осталась в контексте,
   * и лента обязана сказать это прямо, а не нарисовать пометку «мимо».
   */
  | { type: 'side-mark'; kept: boolean }
  /**
...
export interface PersistedConversation {
  ...
  interrupted?: number[]
  /**
   * Эта беседа консоль человека агенту не подаёт.
   *
   * Переживает перезапуск намеренно: отказ отдавать свою консоль — не
   * настроение на один ход, и молча вернуть подачу назавтра значило бы отменить
   * его решение за него.
   */
  shellTailOff?: boolean
  /**
   * Куда вернуть беседу, если боковой вопрос не успел завершиться.
   *
   * Переживает перезапуск по той же причине, что и `resumeAt` рядом: иначе
   * закрытое посреди хода приложение молча оставило бы вопрос в контексте, а
   * пометка над ним продолжала бы обещать, что он туда не попал.
   */
...
export type AgentStreamEvent =
  ...
  /**
   * КТО СЕЙЧАС В ФОНЕ — полный список, а не «ещё одна началась».
   *
   * Движок шлёт его при каждом изменении состава (`background_tasks_changed`),
   * и документация SDK отдельно требует ЗАМЕНЯТЬ набор целиком, а не собирать
   * из пар «началось / кончилось»: одна потерянная пара оставила бы счётчик
   * гореть над пустотой навсегда.
   *
   * Пустой список — законное значение и означает «в фоне никого». Он же
   * приходит от нас при (пере)запуске движка: сам движок при старте не шлёт
   * ничего, и без сброса на экране остался бы список задач прошлой жизни.
   */
  | {
      type: 'background'
  ...
  | {
      ...
      /**
       * После этой команды возврата нет — ПОДПИСЬ на карточке, а не второй
       * гейт. При автопилоте карточки не будет вовсе (пол снят 2026-09-04);
       * когда гейты включены, человек решает, видя цену.
       */
...
export interface AgentCapabilities {
  ...
  images?: boolean
  /**
   * Движок понимает СРЕДНЮЮ ступень допуска — «правки без спроса».
   *
   * Ревью поймало: у Codex и ACP её нет вовсе (флаг до них не доезжает), а чип
   * в баре всё равно вставал в это положение — и после того, как у ступени
   * появился свой глиф, враньё стало читаться без наведения. Где false, цикл
   * допуска идёт мимо середины: «спрашивать» → «автопилот».
   */
  editsAuto?: boolean
  /**
   * Движок умеет БОКОВОЙ ВОПРОС: спросить и не потащить это дальше.
   *
   * Держится на том же форке сессии, что и откат, но обещание другое, поэтому
   * и флаг свой: откат УБИРАЕТ отправленное, боковой вопрос оставляет его на
   * виду и не пускает в следующий ход. Где false — чипа нет вовсе: кнопка,
   * обещающая «мимо контекста» там, где движок так не умеет, — прямое враньё
   * ровно о том, ради чего её нажимают.
   */
  sideAsk?: boolean
  /**
  ...
  stopTask?: boolean
  /**
   * Движок умеет увести идущую работу В ФОН: ход продолжается, задача остаётся.
   *
   * Отдельно от `stopTask`, потому что это противоположное решение: там работу
   * прекращают, здесь — оставляют жить, перестав её ждать. Где false, кнопки
   * нет вовсе: обещать «уйдёт в фон» и оставить человека ждать — хуже, чем не
   * обещать.
   */
...
export interface McpServerRow {
  ...
  tools?: number
  /** Во сколько обходятся описания этих инструментов, если их раскрыть. Считает движок. */
  tokens?: number
  /**
   * Сколько из них лежит в окне ПРЯМО СЕЙЧАС.
   *
   * Движок описания откладывает и подгружает по надобности, поэтому «сколько
   * стоит» и «сколько занято» — разные числа, и раньше мы показывали первое,
   * называя его вторым. Ноль значит «отложены целиком»; поля нет — движок про
   * этот сервер не сказал, и выдумывать за него нельзя.
   */
  loadedTokens?: number
}
