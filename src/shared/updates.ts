...
/** Ответ свежее этого срока переспрашивать незачем. */
export const RECHECK_FRESH_MS = 30_000

/**
 * Спросить ли заново, когда человек открыл окно обновления.
 *
 * Зачем вообще спрашивать. Проверка идёт раз при запуске, а подпись релиза
 * появляется ПОЗЖЕ сборки: её ставит человек ключом, которого нет в CI. На
 * 0.7.3 это вышло так, что окно честно говорило «у релиза нет подписи автора»
 * ещё долго после того, как подпись легла, и переоткрытие ничего не меняло.
 *
 * Почему не всегда. Открыть и закрыть окно дважды подряд — обычное дело, и
 * сеть за это платить не должна: ответ свежее полминуты берём как есть.
 *
 * Почему не при выключенных проверках. «Проверять обновления» выключено —
 * значит Заря не спрашивает сама нигде, включая это окно. Кнопка «Проверить
 * снова» при этом остаётся: человек отказался от фоновых проверок, а не от
 * права спросить руками.
 */
export function shouldRecheck(o: {
  open: boolean
  autoCheck: boolean
  checkedAt?: number
  now: number
  ...
  if (!o.open || !o.autoCheck) return false
  return o.now - (o.checkedAt ?? 0) > (o.freshMs ?? RECHECK_FRESH_MS)
}
