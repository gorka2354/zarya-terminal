...
/**
 * Что считается необратимым — и почему такому не выдают правило «до конца сессии».
 *
 * До 2026-09-04 этот список был ещё и «полом под автопилотом»: такие команды
 * показывались даже при снятых гейтах. Пол убран осознанно (он ловил лишь
 * дословную форму команды и защитой не был), а список остался в двух ролях:
 * подпись «это не отменить» на карточке и запрет на правило «до конца сессии» —
 * такое решают по одному разу.
 *
 * Границы списка проверяются здесь построчно, потому что двигать их в любую
 * сторону — менять смысл того, что человек читает перед нажатием.
 */
...
describe('правила «до конца сессии»', () => {
  ...
  it('ЧТЕНИЕ ВЫВОДА «до конца сессии» не разрешается вовсе', () => {
    /*
     * Правило для не-shell инструментов — это ИМЯ, без аргументов. Для
     * `read_block` оно означало бы «читай вывод любой моей команды до конца
     * беседы», включая все будущие (`id: "last"`), — при том что человек
     * нажимал кнопку на карточке, назвавшей ОДНУ команду. Дальше он в своей же
     * панели делает `gh auth token` или `cat .env`.
     */
    expect(ruleFor('mcp__zarya__read_block', { id: 'b17' })).toBeNull()
    expect(ruleFor('mcp__zarya__read_block', { id: 'last' })).toBeNull()
    // И правило, если бы оно откуда-то взялось, всё равно не сработает.
    expect(matchesRule(['mcp__zarya__read_block'], 'mcp__zarya__read_block', { id: 'b1' })).toBe(
      false
    )
  })
