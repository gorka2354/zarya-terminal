...
describe('«стоит» и «занято» — разные числа', () => {
  ...
  it('НЕПОЛНЫЙ ответ про сервер тоже не даёт числа', () => {
    /*
     * Поле необязательное у каждого инструмента, так что частичный ответ — не
     * выдумка. Заведи мы сервер по первому помеченному — подпись «отложены»
     * относилась бы ко всем его инструментам по данным о части.
     */
    const partial = [
      { name: 'a', serverName: 'zarya', tokens: 300, isLoaded: true },
      { name: 'b', serverName: 'zarya', tokens: 761 },
      { name: 'c', serverName: 'blender', tokens: 7999, isLoaded: false }
    ]
    expect(foldMcpLoaded(partial)).toEqual({ blender: 0 })
  })
