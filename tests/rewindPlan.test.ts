...
describe('staleAgainst', () => {
  ...
  it('путь ДОБАВИЛСЯ, пока человек читал, — тоже расхождение', () => {
    // Соседняя панель тронула ещё один файл: согласие относилось к прежнему
    // набору, и без обратной проверки этот файл перезаписался бы молча.
    expect(staleAgainst([f('a.ts', 'h1')], [f('a.ts', 'h1'), f('b.ts', 'h2')])).toEqual(['b.ts'])
  })
