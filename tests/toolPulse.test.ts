...
describe('Порог тишины не уходит в бесконечность', () => {
  it('одна долгая заминка не выключает предупреждение навсегда', () => {
    /*
     * `gapMax` растёт и не убывает. Без потолка одна пятиминутная заминка
     * делала бы порог четвертьчасовым, и «пульса нет» больше не сказалось бы
     * никогда — предупреждение, которое молчит всегда, это не предупреждение.
     */
    let p = notePulse(undefined, 0, 0)
    p = notePulse(p, 10 * 60_000, 600) // десятиминутный промежуток
    expect(p.gapMax).toBe(10 * 60_000)
    // Порог упёрся в потолок: через шесть минут тишины новость уже есть.
    expect(pulseSilence(p, 10 * 60_000 + 6 * 60_000)).toBeTruthy()
  })
})

describe('pulseAlive — свой порог, мягче', () => {
  it('единственный удар считается за жизнь, пока он свежий', () => {
    // «Жив» — пересказ того, что движок только что сказал, и одного удара для
    // него достаточно. «Мёртв» — утверждение о чужой смерти, и на него нужен
    // измеренный ритм. Отсюда и разные пороги.
    const p = notePulse(undefined, 0, 1)
    expect(pulseAlive(p, 1_000)).toBe(true)
    expect(pulseSilence(p, 10 * 60_000)).toBeNull()
  })

  it('но и он протухает: давний удар жизнь не подтверждает', () => {
    const p = notePulse(undefined, 0, 1)
    expect(pulseAlive(p, PULSE_QUIET_FLOOR_MS + 1)).toBe(false)
  })
})
